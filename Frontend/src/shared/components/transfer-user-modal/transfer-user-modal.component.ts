...
export class TransferUserModalComponent implements OnInit {
  ...
  cargarUsuarios() {
    if (this.tipoUsuario === 'jugador') {
      this.clubService.getJugadoresClub(this.data.clubId)
        .subscribe(res => this.usuarios = res);
    } else {
      this.clubService.getEntrenadoresClub(this.data.clubId)
        .subscribe(res => this.usuarios = res);
    }
  }
  ...
  traspasar() {
    ...
    this.usuarioService.traspasarUsuario(
  ...
  eliminar() {
    ...
    this.usuarioService.eliminarUsuario(this.usuarioSeleccionado.DNI)
      .subscribe({
        next: () => {
          this.usuarioSeleccionado = null;
          this.cargarUsuarios();
